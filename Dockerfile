...
RUN apt-get update && apt-get install -yq libgconf-2-4 gnupg ttf-mscorefonts-installer
...
ADD gradle/wrapper /tmp/gradle/wrapper
ADD gradle.properties /tmp/
ADD gradlew /tmp/
ADD settings.gradle /tmp/
ADD src/main/kotlin /tmp/src/main/kotlin
WORKDIR /tmp/
